...
// Stackryze 托管子域名查询（api-domains.stackryze.com）
async function queryStackryzeWhois(domain) {
  try {
    const url = `https://api-domains.stackryze.com/subdomains/whois?query=${encodeURIComponent(domain)}`;
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 15000);
    let response;
    try {
      response = await fetch(url, {
        ...
        headers: {
          'accept': '*/*',
          'origin': 'https://domain.stackryze.com',
          'referer': 'https://domain.stackryze.com/',
        },
        signal: controller.signal,
      });
    } finally {
    ...
    if (!response.ok) {
      throw new Error(data.error || `Stackryze WHOIS查询失败: ${response.status}`);
    }
    ...
    return {
      ...
      status: data.domainStatus ? [data.domainStatus] : [],
      dnssec: data.dnssec || null,
      raw: data,
    };
  } catch (error) {
    const msg = error.name === 'AbortError' ? 'Stackryze WHOIS查询超时（15秒）' : error.message;
    return {
      success: false,
      error: msg,
      domain: domain,
                    ...
                    <div class="table-responsive" style="max-height: 360px; overflow-y: auto;">
                        <table class="table table-sm table-hover align-middle mb-0">
                            ...
                            <tbody id="dnsheImportList">
                                <tr><td colspan="6" class="text-muted text-center py-4">点击「刷新列表」加载 DNSHE 域名</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
        ...
        function setupEventListeners() {
            ...
            function renderDnsheImportList(items) {
                ...
                if (!dnsheImportItems.length) {
                    tbody.innerHTML = '<tr><td colspan="6" class="text-muted text-center py-4">DNSHE 账号下没有可显示的域名</td></tr>';
                ...
                tbody.innerHTML = dnsheImportItems.map(function(item, index) {
                    ...
                    return '<tr>' +
                        ...
                        '<td>' + (cf ? renderCloudflareBadgeHtml(cf) : '-') + '</td>' +
        ...
        if (domainGroups[categoryId]) {
            ...
            const renderGroup = (categoryData) => {
                ...
                (domain.customNote && domain.customNote.trim() !== '' ?
                    ...
                    '<h5 class="mb-0 domain-title" style="word-break: break-all;"><span class="domain-text" style="line-height: var(--domain-line-height);">' + escapeHtml(domain.name) + '</span></h5>' +
                    '<div class="spacer" style="height: var(--domain-note-spacing);"></div>' +
                    '<div class="domain-meta">' + cloudflareBadgeHtml + '</div>' +
                    '</div>'
                ) +
                '</div>' +
